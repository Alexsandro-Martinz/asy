...
from backend.services.paciente_services import PacienteServices
...
def paciente_create(request):
    ...
    if request.method == "POST":
        ...
        forms = [user_form_instance, paciente_form_instance]

        for form in forms:
            if not form.is_valid():
                messages.add_message(request, messages.ERROR, form.errors.as_text())
                return render(
                    request, "paciente/paciente_create.html", {"forms": forms}
                )
...
def paciente_detail(request, id):
    try:
        ...
        messages.add_message(
            request, messages.ERROR, f"Ocorreu um erro ao carregar o paciente: {str(e)}"
        )
        return redirect("paciente_list")
...
def paciente_update(request, id):
    ...
    if request.method == "POST":
        ...
        if not paciente_form_instance.is_valid():
            messages.add_message(
                request, messages.ERROR, paciente_form_instance.errors.as_text()
            )
            return render(
                request,
                "paciente/paciente_update.html",
                {"form": paciente_form_instance},
            )
